...
ggplot(datos, aes(x = factor(edad_primer_hijo), y = riqueza)) +
     geom_boxplot(fill = "brown4", color = "brown1") +
  labs(title = "Edad del Primer Hijo por riqueza",
       x = "Edad del Primer Hijo",
       y = "Riqueza") +
  theme_minimal()
```
...
data <- datos %>% select(-caseid, -peso, -año, -cluster, -hogar, -SurveyId, -edad_muj, -años_residencia, -ideal_hijos)
...
reg_tree_fit <- reg_tree_spec %>%
  fit(edad_primer_hijo ~ ., data = data)
...
rf_fit <- fit(rf_spec, edad_primer_hijo ~ ., data = data_train)
rf_fit 
