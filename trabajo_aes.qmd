...
[OTROS POSIBLES GRAFICOS (ELEGIR SI DEJR O SACAR)]

```{r psexo_xind_riqueza}
#| label: fig-psexo_xind_riqueza
#| fig-cap: "Gráfico de caja, distribución del índice de riqueza de la mujer por comienzo de relaciones sexuales."
#| fig-align: center
#| fig-height: 3.5
#| fig-width: 5

muestra_psexo %>%
    ggsurvey(aes(x = as.factor(psexo_18), y = indice_riqueza, fill = as.factor(psexo_18))) +
    geom_boxplot(color = "grey") +
    labs(x = "",
         y = "Índice de Riqueza") +
    theme_minimal() +
    scale_fill_viridis_d(name = "Comienzo Relaciones Sexuales",
                         labels = c("Menor a 18 años", "Mayor o igual a 18 años"))
...
```{r psexo_xn_parejas}
#| label: fig-psexo_xn_parejas
#| fig-cap: "Gráfico barras, Estimación de la proporción de mujeres que comienzan a tener relaciones sexuales antes de los 18 años por la cantidad de parejas que han tenido."
#| fig-align: center
#| fig-height: 3.5
#| fig-width: 5

prop_n_parejas_tv <- muestra_psexo  %>%
    mutate(n_parejas_rango = case_when(
        n_parejas == 1 ~ "1",
        n_parejas >= 2 & n_parejas <= 5 ~ "2-5",
        n_parejas >= 6 & n_parejas <= 10 ~ "6-10",
        n_parejas >= 11 & n_parejas <= 20 ~ "11-20",
        n_parejas > 20 ~ "Más de 20"
    )) %>%
    group_by(n_parejas_rango) %>%
    summarise(mayor_18 = survey_mean(psexo_18), menor_18 = 1 - survey_mean(psexo_18)) %>%
    select(n_parejas_rango, mayor_18, menor_18) %>%
    pivot_longer(cols = c("mayor_18", "menor_18"),
                 names_to = "edad",
                 values_to = "proporcion") %>% 
    mutate(n_parejas_rango = factor(n_parejas_rango, levels = c("1", "2-5", "6-10", "11-20", "Más de 20")))

prop_n_parejas_tv %>%
    ggplot(aes(x = n_parejas_rango, y = proporcion, fill = edad)) +
    geom_bar(stat = "identity", position = "fill") +
    theme_minimal() +
    labs(x = 'Número de Parejas', y = '') +
    scale_fill_viridis_d(name = "Comienzo Relaciones Sexuales",
                         labels = c("Mayor o igual a 18 años", "Menor a 18 años")) +
    theme(legend.position = 'top')
```
...
La busqueda se realiza al igual que para el modelo de visualizacion de importancias mediante *10-fold cross-validation*. En la @fig-tune_bagging_phijo y @fig-tune_rf_phijo se puede visualizar los valores de *RMSE* y *RSQ* para los modelos de *Bagging* y *Random Forest* respectivamente. En ambos gráficos parece bastante marcado una mejor *performance* para valores de *min_n* cercano a los $400$, pareciendo la grilla propuesta adecuada para este problema.
...
[COMENTAR TODA LA INTERPRETACION E IMPORTANCIA DE LAS VARIABLES
GRAFICOS DE ABAJO]
...
[PROBAR SI SE PUEEDE A GEOM_RUG PONERLE UN JITTER]
...
Previo a la división de los datos se procede por filtrar observaciones con valores extremadamente atípicos o que podrian carecer de sentido, en este caso se toma como limite inferior de la edad a la primera relación sexual los 9 años.
...
En la @fig-vip_psexo se observa la importancia de las variables consideradas en el modelo seleccionado. Al igual qque en la @fig-vip_phijo, variables relacionadas con la educación o riqueza del hogar tienden a tener mayor importancia. De todas formas existen algunas diferencias sobre todo en el orden de importancia, aquí vemos que la cantidad de parejas de la mujer parecería incidir más en la edad a la que comienzan a tener relaciones sexuales. Además cabe destacar el aumento de importancia en este caso de la frecuencia con la que la mujer ve noticias, esta información suele ser de interés en este tipo de estudios.  
...
En esta instancia se vuelve a trabajar integramente con '*tidymodels*', pero en este caso se opta por evaluar la eficiencia de modelos basados en árboles: *Bagging*, *Random Forest* y además se ajusta un modelo *Logístico*, nuevamente para los modelos basados en árboles se realiza un tuneo de hiperparámetros.
...
```{r}
fit_test_psexo %>% 
  conf_mat(truth = psexo_18, estimate = .pred_class) %>% autoplot()
```
