...
mutate(v000 = as.factor(case_when(
  ...
  v613 = as.numeric(case_when(v613 %in% 96:99 ~ NA, T~v613)),
...
desc_variables <- c(
  ...
  'Indicador de riqueza del hogar',
...
rec_variables <- c('10 paises',' ',paste0(min(datos_dhs$v007),'-',max(datos_dhs$v007)),paste0(min(datos_dhs$v012),'-',max(datos_dhs$v012)),'1:Urbano, 2:Rural','0:No, 1:Si','0:No, 1:Si',paste0(min(datos_dhs$v133,na.rm = T),'-',max(datos_dhs$v133,na.rm = T)),paste0(min(datos_dhs$v136),'-',max(datos_dhs$v136)),'0:Nada, 1:Ocasional, 2:Todos los días','0:Nada, 1:Ocasional, 2:Todos los días','0:Nada, 1:Ocasional, 2:Todos los días','1:Más Pobre, 2:Pobre, 3:Medio, 4:Rico, 5:Más Rico', paste0(min(datos_dhs$v191,na.rm = T),'-',max(datos_dhs$v191,na.rm = T)),paste0(min(datos_dhs$v212,na.rm = T),'-',max(datos_dhs$v212,na.rm = T)),'0: Ninguno, 1:Folklórico, 2:Tradicional, 3:Moderno','0: Nunca, 1:Alguna vez Casada',paste0(min(datos_dhs$v525,na.rm = T),'-',max(datos_dhs$v525,na.rm = T)),'0: No, 1: Si',paste0(min(datos_dhs$v836,na.rm = T),'-',max(datos_dhs$v836,na.rm = T)))
...
```{r}
datos_phijo <- datos_dhs %>% filter(edad_phijo >= 11) %>% na.omit()
```

```{r}
dis_muestra <- datos_phijo %>%
              as_survey_design(ids=psu,strata=estrato, weights=ponderador, nest=TRUE)

pop_fem <- read.csv('datos/population.csv') 
datos_pop <- datos_dhs %>% merge(pop_fem%>%select(-anio),by=c('pais'),how='left') %>% group_by(pais) %>% mutate(pop_rep= (ponderador/sum(ponderador)) *pop_fem)
```
...
dis_muestra %>% filter(anio_educ<22) %>%
  ggsurvey(aes(x = edad_phijo, color = anio_educ, group=anio_educ)) + geom_density(linewidth=0.8) +
  labs(x = "Edad al Primer Hijo",
       y = "Frecuencia") +
  theme_minimal() + scale_color_viridis_c(name = "Años de Educación",limits = c(0, 22),oob = scales::squish)
```
...
dis_muestra %>% mutate(anio_edu_15=as.factor(ifelse(anio_educ>=15,1,0))) %>%
...
dis_muestra %>% mutate(indice_riqueza=as.factor(cut(indice_riqueza,breaks = quantile(datos_phijo$indice_riqueza, probs = seq(0, 1, 0.2)),include.lowest =T))) %>%
 ggsurvey(aes(y =edad_phijo, x=	indice_riqueza,fill=zona)) +
...
dis_muestra %>% mutate(n_parejas=as.factor(case_when(n_parejas == 1 ~ '1', n_parejas == 2 ~ '2', n_parejas == 3 ~ '3', 
n_parejas >= 4 ~ '4 o más'))) %>%
 ggsurvey(aes(y = edad_phijo, x= n_parejas,fill=con_antic))  +  geom_boxplot(color = "grey") +
  labs(x = "Cantidad de parejas sexuales",
       y = "Edad al Primer Hijo") +
  theme_minimal() + scale_fill_viridis_d(name="Conocimiento de Métodos")
```
